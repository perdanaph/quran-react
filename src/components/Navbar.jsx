    ...
    <Box className='fixed top-0 left-0 right-0 z-50'>
      ...
      >
        ...
        <Flex flex={{ base: 1 }} justify={{ base: 'center', md: 'start' }}>
          <Text textAlign={useBreakpointValue({ base: 'center', md: 'left' })} fontSize={20} color={useColorModeValue('gray.800', 'white')}>
            QURAN
          </Text>
      ...
      {NAV_ITEMS.map(navItem => (
        <Box key={navItem.label}>
          <Popover trigger={'hover'} placement={'bottom-start'}>
            <PopoverTrigger>
              <NavLink className={({ isActive }) => (isActive ? 'text-teal-500' : '')} to={navItem.href}>
                {navItem.label}
              </NavLink>
            </PopoverTrigger>
...
const MobileNavItem = ({ label, children, href }) => {
  ...
  return (
    <Stack spacing={4} onClick={children && onToggle}>
      <NavLink className={({ isActive }) => (isActive ? 'text-teal-500' : '')} to={href}>
